...
bool BQ27441::begin(TwoWire *i2c)
{
  thisWire=i2c;   
...
{
  int16_t timeout = BQ72441_I2C_TIMEOUT;  
  thisWire->beginTransmission(_deviceAddress);
  thisWire->write(subAddress);
  thisWire->endTransmission(true);
  
  thisWire->requestFrom(_deviceAddress, count);
  
  for (int i=0; i<count; i++)
  {
    dest[i] = thisWire->read();
  }
